...
(* application of functions in the environment *)
let indir_call_ref_step weight (generate : hole_info -> exp) (hole : hole_info) (acc : rule_urn) =
  ...
  let gamma_refs : (var * size_ty) list = List.filter_map
    (fun v -> match TypeUtil.exact_ty_produces v.var_ty hole.ty hole.env with (* look for exact match - we cannot "size up" on an application of something in the environment *)

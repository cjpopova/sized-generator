...
let is_exact_ty maybe target = (* use exact match *)
  gen_compare_ty maybe target (fun sexp1 sexp2 -> sexp1 = sexp2)
...
let rec ty_unify_producer maybe target =
  match maybe with
  | TyArrow (doms, cod) -> 
    if is_size_subtype_ty cod target then (* this part doesn't care about size variable names*)
...
and ty_produces maybe target (env : env) =
  let smaller_env = List.filter (fun v -> not (is_size_subtype_ty v.var_ty maybe)) env in
  (* NOTE: smaller env is hack to (mostly?) prevent infinite loops in reachability
   by removing `maybe` from the candidates for generating its arguments*)
  match ty_unify_producer maybe target with
  | Some TyArrow (doms, cod) -> 
    if List.for_all (fun dom -> (List.exists (fun var -> reachable var.var_ty dom smaller_env) smaller_env)) doms
      then Some (TyArrow(doms, cod))
      else None
  | _ -> None
and reachable maybe t env = is_size_subtype_ty maybe t || Option.is_some (ty_produces maybe t env)

(* a variant of ty_produces that doesn't do size variable unification (requires exact match) *)
let exact_ty_produces maybe target env = 
  let smaller_env = List.filter (fun v -> not (is_size_subtype_ty v.var_ty maybe)) env in (* same hack as above *)
  match maybe with
  | TyArrow (doms, cod) -> 
    if is_exact_ty cod target (* exact match on codomain *)
      && List.for_all (fun dom -> (List.exists (fun var -> reachable var.var_ty dom smaller_env) smaller_env)) doms (* these doms are the original and not unified *)
      then Some maybe (* returns the exact same type back *)
      else None
  | _ -> None
...
(************************ SIZE ALGEBRA DOCUMENTATION ***********************************)
(* Pseudocode for reachable, produces. sigma is the target type

reachable sigma t = is_size_subtype_ty t sigma || (produces sigma t)
produces sigma t = 
  TyArrow(doms, cod) = t
  is_size_subtype_ty cod sigma && (* this part doesn't care about size variable names*)
...
------------------------------------
the tutorial type system doesn't have "sizing up". for consistency, sizing up should be allowed on everything
except recursive functions. there is probably an exception for mutual recursion as well
